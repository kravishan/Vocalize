...
app.post('/refine-review', async (req, res) => {
  try {
    ...
    const prompts = [
      ...
      "If user ask to refine only one part of the review like rewrite only one insident, you need to refine only that insident. You dont need to refine the whole review. keep the other parts like previous and only change the part what user ask for. You should send whole review back but change the only sentense which user asked. This is a super must must",
